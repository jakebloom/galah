...
/**
 * Loads the OAuth credentials, refreshes them if necessary, creates a client.
 * @return {Promise<Auth.OAuth2Client>} the oauth client
 */
async function getOAuthClient(): Promise<Auth.OAuth2Client> {
  ...
  const file = admin
    .storage()
    .bucket(CREDENTIALS_BUCKET_NAME.value())
    .file(CREDENTIALS_FILE_NAME.value());
  ...
  if (
    ...
    oauth2Client.credentials.expiry_date < (new Date().valueOf())
  ) {
...
/**
 * Sends the report to a given email address.
...
export async function sendEmail(email: string) {
  ...
  const emailLines = [
    ...
    "",
    "Hi 👋 Jake here.\n",
    "Thanks for signing up to get our report " +
    "detailing the best performing video ads on the internet!\n",
    "If you have any questions or feedback, " +
    "please feel free to reply and ask away!\n",
    "Enjoy,\nJake Bloom\nFounder\nGalah Labs LLC",
    "",
    "--content_boundary",
